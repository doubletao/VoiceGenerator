...
static void dbg_wave_header(WAVEHDR * buf)
{
	dbg("-----\n");
...
static int open_rec_device(int dev, WAVEFORMATEX *format, HANDLE thread, HWAVEIN *wave_hdl_out)
{
	MMRESULT res;
	HWAVEIN wi = NULL;
	WAVEFORMATEX fmt;
...
static void close_rec_device(HWAVEIN wi)
{
	if(wi != NULL) {
		waveInClose(wi);
	}
...
static int start_record_internal(HWAVEIN wi, WAVEHDR *header, unsigned int bufcount)
{
	MMRESULT res;
	unsigned int i;
...
static int open_recorder_internal(struct recorder * rec, unsigned int dev, WAVEFORMATEX * fmt)
{
